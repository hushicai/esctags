...
exports.parse = function (args) {
    ...
    function getTagsFromMemberAssignmentNode(node,  paths) {
        ...
        function getTagsFromLeft(node) {
            ...
            accessPaths = util.getMemberExpressionPaths(left);
            ...
            var tag = getTag({
                name: name,
                type: (rightType === Syntax.ObjectExpression
                    || rightType === Syntax.FunctionExpression)
                    ? Type.CONTEXT
                    : Type.PROPERTY,
    ...
    function getTag(config) {
        ...
        // 去重
        var temp = config.paths.slice(0);
        temp.push(config.name);
        temp = temp.join('::');
        if (tagsMap[temp]) {
            return null;
        }
    ...
    estraverse.traverse(ast, {
        enter: function (node, parent) {
            ...
            var paths = util.getPaths(parents);
            if (util.isScopeNode(node)) {
                var currentScope = scopeManager.acquire(node);
                var d = util.getScopeName(node, parents);
                var name = d.name;
                ...
                tags = tags.concat(getTagsInScope(currentScope, paths));
            }
